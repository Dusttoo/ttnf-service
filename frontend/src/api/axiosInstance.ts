...
apiClient.interceptors.response.use(
    (response) => {
        const isBackgroundRequest = response.config.headers?.isBackgroundRequest === 'true';
...
export const axiosWithTimeout = (config: AxiosRequestConfig, timeout = TIMEOUT, isBackgroundRequest = false) => {
    return withTimeout(
        apiClient({
            ...config,
            headers: {
